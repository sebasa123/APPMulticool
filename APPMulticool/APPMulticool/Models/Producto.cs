...
namespace APPMulticool.Models
{
    public class Producto
    {
        ...
        public async Task<bool> AddProducto()
        {
            try
            {
                string RouteSuffix = string.Format("Productos/PostProducto");
                ...
                Request.AddJsonBody(
                    new
                    {
                        ...
                        fktipoProd = this.FKTipoProd,
                        estadoProd = this.EstadoProd
                    });
        ...
        public async Task<ObservableCollection<Producto>> GetProductoListByName()
        {
            try
            {
                string RouteSuffix = string.Format("Productos/GetProductoListByNombre?pNombre={0}", this.NombreProd);
                string URL = Services.APIConnection.ProductionURLPrefix + RouteSuffix;
                RestClient client = new RestClient(URL);
                Request = new RestRequest(URL, Method.Get);
                Request.AddHeader(Services.APIConnection.ApiKeyName,
                ...
                if (statusCode == HttpStatusCode.OK)
                {
                    var NombreProdLista = JsonConvert.DeserializeObject<ObservableCollection<Producto>>(response.Content);
                    return NombreProdLista;
                }
                else
                {
        ...
        public async Task<bool> DeleteProducto(int pID)
        {
            ...
            {
                string RouteSuffix =
                     string.Format("Productos/DeleteProducto/{0}", pID);
                string URL = Services.APIConnection.ProductionURLPrefix + RouteSuffix;
                RestClient client = new RestClient(URL);
        ...
        public async Task<ObservableCollection<Producto>> GetProductos()
        {
            try
            {
                ...
                if (statusCode == HttpStatusCode.OK)
                {
                    var ProductoLista = JsonConvert.DeserializeObject<ObservableCollection<Producto>>(response.Content);
                    return ProductoLista;
                }
                else
                {
        ...
        public async Task<Producto> GetProductoID(int pID)
        {
            try
            {
                string RouteSufix = string.Format("Productos/{0}", pID);
